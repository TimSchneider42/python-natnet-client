...
class NatNetClient:
    ...
    def connected(self):
        return (
            self.__command_socket is not None
            and self.__data_socket is not None
            and self.__server_info is not None
        )
    ...
    def __create_data_socket(self, port):
        ...
        if self.__use_multicast:
            result.setsockopt(
                ...
                socket.inet_aton(self.__multicast_address)
                + socket.inet_aton(self.__local_ip_address),
            )
        return result

    def __socket_thread_func(
        self,
        in_socket: socket.socket,
        recv_buffer_size: int = 64 * 1024,
    ...
    def __process_message(self, buffer: PacketBuffer):
        ...
        if len(buffer.data) - 4 != packet_size:
            print(
                f"Warning: actual packet size ({len(buffer.data) - 4}) not consistent with packet size in the "
                f"header ({packet_size})"
            )
        if message_id == self.NAT_SERVERINFO:
            self.__server_info = ServerInfo.read_from_buffer(
            ...
            if self.__current_protocol_version is None:
                print(
                    f"Warning: dropping packet of type {message_id} as server info has not been received yet."
                )
            if message_id == self.NAT_FRAMEOFDATA:
                data_frame = DataFrame.read_from_buffer(
                    buffer, self.__current_protocol_version
                )
    ...
    def connect(self, timeout: float = 5.0):
        if not self.connected:
            ...
            while self.__server_info is None:
                ...
                self.__process_socket(
                    self.__command_socket, send_keep_alive=not self.__use_multicast
                )
    ...
    def run_async(self):
        if not self.running_asynchronously:
            ...
            self.__data_thread = Thread(
                target=self.__socket_thread_func, args=(self.__data_socket,)
            )
            self.__data_thread.start()

            # Create a separate thread for receiving command packets
            self.__command_thread = Thread(
                target=self.__socket_thread_func,
                args=(self.__command_socket,),
    ...
    def can_change_protocol_version(self) -> bool:
        if self.__server_info is not None:
            return (
                self.__server_info.nat_net_protocol_version >= Version(4)
                and not self.__use_multicast
            )
        return False
    ...
    def protocol_version(self, desired_version: Version):
        ...
        if (
            self.can_change_protocol_version
            and desired_version != self.__current_protocol_version.truncate(2)
        ):
